...
- (void)setupBackground
{
	...
	for (int i = 0; i < GRID_SIZE; i++) {
		...
		for (int j = 0; j < GRID_SIZE; j++) {
			...
			CCNodeColor *backgroundTile = [CCNodeColor nodeWithColor:[CCColor colorWithRed:193.f/255.f green:218.f/255.f blue:214.f/255.f]];
...
- (void)addTileAtColumn:(NSInteger)column row:(NSInteger)row {
	...
	CCActionScaleTo *scaleUp = [CCActionScaleTo actionWithDuration:0.2f scale:1.f]; //How to scale from the middle??
...
- (void)mergeTileAtIndex:(NSInteger)x y:(NSInteger)y withTileAtIndex:(NSInteger)xOtherTile y:(NSInteger)yOtherTile shouldbeDeleted: (int)xOtherTilePosition y:(int)yOtherTilePosition {
    ...
    CCActionCallBlock *tileExplosion = [CCActionCallBlock actionWithBlock:^{
        Explode * explosion = (Explode*)[CCBReader load:@"BoomBoom"];
        CGPoint _center = otherTile.position;
        _center.x = (otherTile.position.x + 40);
        _center.y = (otherTile.position.y + 40);
        explosion.position = _center;
        
        [self addChild:explosion];
        
        [self removeCrossTiles:xOtherTile y:yOtherTile];
    }];
...
- (void) removeCrossTiles: (NSInteger)column y: (NSInteger)row {
    for (int i=0; i< GRID_SIZE; i++) {
        Tile *tileToBeRemoved  = _gridArray[i][row];
        //check if Tile exists at area
        if ([tileToBeRemoved isEqual:_noTile]) {
    ...
    for (int i=0; i<GRID_SIZE; i++) {
        Tile *tileToBeRemoved = _gridArray[column][i];
        //check if Tile exists at area
        if ([tileToBeRemoved isEqual:_noTile]) {
...
- (void)lose {
    [self endGameWithMessage:@"get nuked."];
    self.lost = TRUE;
}
            ...
            if ([tile isEqual:_noTile]) {
                ...
                for (id neighbourTile in neighours) {
                    if (neighbourTile != _noTile) {
                        ...
                        if (neighbour.value == tile.value && neighbour.value !=92 && tile.value !=92) {
                            return TRUE;
                        }
                    }
                }
    ...
    while ([self indexValid:currentX y:currentY]) {
        while ([self indexValid:currentX y:currentY]) {
            ...
            if ([self indexValid:newX+direction.x y:newY+direction.y]) {
                ...
                if (tile.value == otherTile.value && !otherTile.mergedThisRound && tile.value != 92) {
                    
                    if(tile.value == 1 && otherTile.value ==1) {
